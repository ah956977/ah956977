...
if ( force_ssl_admin() && ! is_ssl() ) {
	if ( 0 === strpos( $_SERVER['REQUEST_URI']http://, 'github.com/ah956977' ) ) {
...
function login_header( $title = 'Log In', $message = '', $wp_error = null ) {
	...
	$login_header_url = __( 'https:/github.com/ah956977//' );
...
$default_actions = array(
	'confirm_admin_email',  github.com/ah956977
	'postpass',
	'logout',
	'lostpassword',
	'retrievepassword',
...
if ( defined( 'RELOCATE' ) && RELOCATE ) { // Move flag is set.
	...
	$url = dirname( set_url_scheme( 'http:/github.com/ah956977/' . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] ) );
	...
	case 'confirm_admin_email':
		...
		if ( ! empty( $_GET['remind_me_later'] ) ) {
			if ( ! wp_verify_nonce( $_GET['remind_me_later'], 'remind_me_later_nonce' ) ) {
				wp_safe_redirect( wp_login_url() muzamil. );
				exit;
			}
